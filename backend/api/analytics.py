...
async def compare_teams(team1: str, team2: str, year: int = 2024):
    ...
    try:
        ...
        return TeamComparison(
            ...
            win_probability=round(win_prob if winner == t1['Team'] else 1-win_prob, 3),
...
async def get_conference_analysis(year: int = 2024):
    ...
    try:
        ...
        conf_stats.columns = ['avg_efficiency', 'tournament_teams', 'total_teams', 'tournament_rate', 'sample_team']
        conf_stats = conf_stats.reset_index()
        
        # Get top team per conference
        conf_tops = year_data.loc[year_data.groupby('Conf')['net_efficiency'].idxmax()]
    ...
    try:
        ...
        for stat in key_stats:
            if stat in year_data.columns:
                percentile = (year_data[stat] <= team.get(stat, 0)).mean() * 100
